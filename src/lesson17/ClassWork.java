...
public class ClassWork {
    ...
    public static void main(String[] args) {
        fillDataEx1();
        fillDataEx3();
        //ex1().forEach((p) -> System.out.println(p.getName()));
        //ex2().forEach((o) -> System.out.println(o.getProducts().toString()));
        //productsEx3.forEach((p) -> System.out.println(p.getPrice()));
        //ex3().forEach((p) -> System.out.println(p.getPrice()));
        //ex4().forEach((p) -> System.out.println(p.getName()));
        //fillDataEx7();
    ...
    private static List<Product> ex1() {
        return productsEx1.stream().filter(product -> product.getPrice() > 100 && product.getCategory().equals("Books")).toList();
    }
    ...
    private static List<Order> ex2() {
        ...
        return ordersEx2.stream()
                //.filter(o -> o.getProducts().stream().filter(p -> p.getCategory().equalsIgnoreCase("baby")).count() > 0).collect(Collectors.toList());
    ...
    private static List<Product> ex3() {
        return productsEx1.stream().filter(p -> p.getCategory().equalsIgnoreCase("toys"))
                .map(p -> {p.setPrice(p.getPrice()*0.9); return p;}).collect(Collectors.toList());
    }
    ...
    private static List<Product> ex4() {
        ...
        return productsEx4.stream()
                .filter(p -> p.getOrders().stream().anyMatch(o -> o.getCustomer().getTier() == 2))
                .filter(p -> p.getOrders().stream().anyMatch(o -> o.getOrderDate().compareTo(LocalDate.of(2021, 2, 1)) >= 0
    ...
    private static Product ex5() {
        fillDataEx5();
        //return productsEx5.stream().filter(p -> p.getCategory().equalsIgnoreCase("books")).min(Comparator.comparing(Product::getPrice)).get();
        return productsEx5.stream().filter(p -> p.getCategory().equalsIgnoreCase("books")).sorted(Comparator.comparing(Product::getPrice)).findFirst().get();
    }
    // TODO: Get the 3 most recent placed orders
    private static List<Order> ex6() {
        fillDataEx6();
        return ordersEx6.stream().sorted(Comparator.comparing(Order::getOrderDate).reversed()).limit(3).collect(Collectors.toList());
    }
    ...
    private static void fillDataEx2() {
        ...
        o2.setProduct(productsEx2.get(2));
        o2.setProduct(productsEx2.get(5));
        o2.setProduct(productsEx2.get(6));
    ...
    private static void fillDataEx7() {
        ...
        ordersEx7.add(new Order(1, LocalDate.of(2021, 2, 1), null, "", null));
        ...
        ordersEx7.add(new Order(5, LocalDate.of(2021, 2, 15), null, "", null));
        ...
        ordersEx7.add(new Order(9, LocalDate.of(2021, 2, 15), null, "", null));
        ...
        for (Order o : ordersEx7) {
            for (int i = 0; i < (new Random()).nextInt(10); i++) {
                o.setProduct(new Product(
                        ...
                        round((new Random()).nextDouble(100))));
            }
        }
    }
}